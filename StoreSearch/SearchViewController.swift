...
extension SearchViewController: UISearchBarDelegate {
    ...
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        if !searchBar.text!.isEmpty {
            ...
            let dataTask = session.dataTask(with: url, completionHandler: { data, responce, error  in
                
                if let error = error {
                    print("Failure! \(error.localizedDescription)")
                } else if let httpResponce = responce as? HTTPURLResponse,
                              httpResponce.statusCode == 200 {
                    if let data = data {
